...
function VerticalDock() {
  ...
  return (
    <nav className="glass-nav w-20 fixed left-0 top-0 bottom-0 z-50 flex flex-col items-center py-8 justify-between" aria-label="Main Navigation">
      {/* Brand Logo */}
      <div className="flex flex-col items-center gap-2 group cursor-pointer" aria-hidden="true">
      ...
      <div className="flex flex-col gap-8">
        ...
        <Link 
          to="/attendee" 
          aria-label="Mobile VIP Concierge"
          className={`flex items-center justify-center w-12 h-12 rounded-2xl transition-all duration-300 ${
      ...
      <div className="w-8 h-8 rounded-full bg-slate-800 border border-slate-700" aria-hidden="true"></div>
    </nav>
  );
}
...
function LoadingScreen() {
  return (
    <div className="h-screen w-full flex items-center justify-center text-cine-accent bg-cine-dark tracking-widest text-xs uppercase animate-pulse">
      Initializing Sector...
    </div>
  );
}
    ...
    <BrowserRouter>
      ...
      */}
      <div className="h-screen w-screen bg-cine-dark text-slate-200 flex font-sans overflow-hidden">
        <VerticalDock />
        <main className="flex-1 w-full relative ml-20" aria-live="polite">
